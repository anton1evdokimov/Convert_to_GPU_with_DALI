...
class ModelTRT:
    ...
    def __init__(self, serialized_engine: bytes, num_total_classes: int, desired_classes: Optional[List[int]],
                is_end2end: bool,
                img_buffer_kind_gpu: bool = False,
        ...
        self.img_buffer_kind_gpu = img_buffer_kind_gpu
        self.t_inf = 0
        ...
        for binding in engine:
            # if we have gpu-array as an input, we do not need to allocate memory on the device and host
            # because we get acces to the gpu-array buffer in _infer_img_from_device()
            print("binding", binding)
            if self.img_buffer_kind_gpu and engine.binding_is_input(binding):
                self.bindings = [None] # this field will be rewritten during __call__ 
                continue
    ...
    def __call__(self, img) -> Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]]:
        """
        ...
        # from gpu buffer
        if self.img_buffer_kind_gpu:
            preds = self._infer_img_from_device(img)
        else: 
            preds = self._infer_img_from_host(img)
        if self.is_end2end:
    ...
    def _infer_img_from_device(self, img: "cvcuda.Tensor") -> List[np.ndarray]:
        # set binding for an input gpu tensor
        # img = cp.asarray(img.cuda()).astype(np.float16)
        print("self.bindings[0]", self.bindings, self.inputs)
        print("img.cuda().__cuda_array_interface__['data'][0]", img.cuda().__cuda_array_interface__['data'][0])
        self.bindings[0] = img.cuda().__cuda_array_interface__['data'][0]
        # Infer
    ...
    def _infer_img_from_host(self, img: np.ndarray) -> List[np.ndarray]:
    ...
    def _postprocess(self, preds: List[np.ndarray]) -> Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]]:
        ...
        # print("num_dets, boxes, scores, cls_inds = preds", preds)
