...
# @cuda.jit
def process(img):
    print("self._put_img(img)")
    print(img)
    print("process(img: np.ndarray)")
    # self._put_img(img) # type: ignore
    # # self._resize_()
    # print("# self._resize_()")
    # self._pad_()
...
# bpg = int(np.ceil((32)/tpb))  # блоков на грид 
...
class ModelDetect:
    ...
    def __call__(self, img: np.ndarray) -> Tuple[cp.ndarray, cp.ndarray, cp.ndarray]:
        t0 = perf_counter()
        print("perf_counter()")
        # img = process[bpg, tpb](cuda.to_device(img))
        t1 = perf_counter()
        self.t_prep = t1 - t0
        print("self.t_prep", self.t_prep)
