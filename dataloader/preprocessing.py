...
class ImgPipeline:
    ...
    def process(self, img: cp.ndarray) -> cp.ndarray:
        img = cp.array(img)
        self._put_img(img)
        self._resize_()
        self._pad_()
    ...
    def _put_img(self, img: cp.ndarray) -> None:
        self.img = img
    ...
    def _define_scaling_parameters(self)-> None:
        ...
        if self.scale > 1:
            self.h, self.w = (cp.round(self.h0 / self.scale)), int(cp.round(self.w0 / self.scale))
        else:
        ...
        self.top, self.bottom = cp.int32(cp.round(dh - 0.1)), cp.int32(cp.round(dh + 0.1))
        self.left, self.right = cp.int32(cp.round(dw - 0.1)), cp.int32(cp.round(dw + 0.1))
        return None
    ...
    @staticmethod
    def data_decode(data: bytes)-> Tuple[str, cp.ndarray]:
        frame_id, img_bytes = data.split(b';', 1)
        img = cvcuda.imdecode(cp.frombuffer(img_bytes, cp.uint8), cv2.IMREAD_COLOR)
